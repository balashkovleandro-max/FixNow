...
<nav class="fixed inset-x-0 bottom-0 z-[70] border-t border-white/70 bg-white/88 px-3 pt-2 shadow-[0_-18px_48px_rgba(30,64,175,0.12)] backdrop-blur-2xl md:hidden" style="padding-bottom: max(0.6rem, env(safe-area-inset-bottom));" aria-label="Мобилна навигация">
    <div class="mx-auto grid max-w-md grid-cols-5 gap-1">
        ...
        <a href="{{ route('business.landing') }}" class="{{ $itemBase }} {{ request()->routeIs('business.landing') ? $itemActive : $itemIdle }}">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 20V8l8-4 8 4v12"/><path d="M9 20v-7h6v7" stroke-linecap="round" stroke-linejoin="round"/></svg>
        ...
        <a href="{{ route('bon.tools') }}" class="{{ $itemBase }} {{ request()->routeIs('bon.tools') || request()->routeIs('business.insights.*') ? $itemActive : $itemIdle }}">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19V5"/><path d="M8 17V9M12 17V7M16 17v-5M20 17V4" stroke-linecap="round"/></svg>
            <span>Инструменти</span>
        </a>
        <a href="{{ route('plans') }}" class="{{ $itemBase }} {{ request()->routeIs('plans') ? $itemActive : $itemIdle }}">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7h16M4 12h16M4 17h10" stroke-linecap="round"/></svg>
            <span>Планове</span>
        </a>
