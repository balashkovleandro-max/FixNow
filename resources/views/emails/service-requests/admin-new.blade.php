...
<head>
    <meta charset="UTF-8">
    <title>Нова заявка за оферта във BON</title>
</head>
<body style="margin:0;background:#f6f8fb;color:#0f172a;font-family:Arial,sans-serif;">
    <div style="max-width:680px;margin:0 auto;padding:28px 18px;">
        <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:18px;padding:26px;">
            ...
            <h2 style="margin:0 0 10px;font-size:18px;">Автоматично назначени бизнеси</h2>
            @forelse($assignments as $assignment)
                <p style="margin:0 0 8px;color:#334155;">
                    <strong>{{ $assignment->business?->business_name ?: $assignment->business?->name ?: 'Изтрит бизнес' }}</strong>
                    <span style="color:#64748b;">· {{ $assignment->status }}</span>
                </p>
            @empty
                <p style="margin:0 0 18px;color:#b45309;">Няма автоматично назначени бизнеси.</p>
            @endforelse
