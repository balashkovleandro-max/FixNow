...
<body class="fn-premium-page min-h-screen overflow-x-hidden pb-24 text-white md:pb-0">
    <div class="fixed inset-0 -z-10 bg-[radial-gradient(circle_at_16%_18%,rgba(249,115,22,0.22),transparent_28%),radial-gradient(circle_at_82%_10%,rgba(245,158,11,0.18),transparent_30%),linear-gradient(180deg,#030712_0%,#061426_52%,#020812_100%)]"></div>
    ...
    <header class="border-b border-white/10 bg-[#030712]/80 backdrop-blur-2xl">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
            ...
            <nav class="hidden items-center gap-6 md:flex">
                <a href="{{ url('/') }}" class="text-sm font-semibold text-white/70 hover:text-orange-300">Начало</a>
                <a href="{{ route('services.index') }}" class="text-sm font-semibold text-white/70 hover:text-orange-300">Услуги</a>
                <a href="{{ route('businesses.index') }}" class="text-sm font-semibold text-white/70 hover:text-orange-300">Изпълнители</a>
                <a href="{{ route('business.landing') }}" class="text-sm font-semibold text-white/70 hover:text-orange-300">За изпълнители</a>
                <a href="{{ url('/contact') }}" class="text-sm font-semibold text-white/70 hover:text-orange-300">Контакт</a>
            </nav>
            <a href="{{ route('register') }}" class="inline-flex min-h-11 items-center rounded-2xl bg-gradient-to-r from-orange-500 to-amber-400 px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-orange-600/25">
                Регистрация
            </a>
        </div>
    </header>
    ...
    <main class="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_480px] lg:px-8">
        <section class="hidden lg:block">
            <div class="max-w-xl">
                <p class="mb-4 inline-flex rounded-full border border-orange-300/20 bg-orange-300/10 px-4 py-2 text-sm font-semibold text-orange-100">Добре дошли отново</p>
                <h1 class="text-5xl font-black leading-tight">
                    Управлявайте профила си в
                    <span class="bg-gradient-to-r from-orange-300 via-amber-300 to-orange-500 bg-clip-text text-transparent">FixNow.bg</span>
                </h1>
                ...
                <div class="mt-8 grid grid-cols-2 gap-4">
                    <div class="rounded-3xl border border-white/10 bg-white/10 p-5 backdrop-blur-xl">
                        <p class="text-3xl font-black text-orange-300">24/7</p>
                        <p class="mt-2 text-sm text-white/60">достъп до профила</p>
                    </div>
                    <div class="rounded-3xl border border-white/10 bg-white/10 p-5 backdrop-blur-xl">
                        <p class="text-3xl font-black text-amber-200">EU</p>
        ...
        <section class="rounded-[32px] border border-white/10 bg-white/10 p-3 shadow-2xl shadow-black/30 backdrop-blur-2xl">
            <div class="rounded-[26px] border border-white/10 bg-slate-950/70 p-6 sm:p-8">
                <p class="text-sm font-black uppercase tracking-[0.25em] text-orange-200/80">Вход</p>
                ...
                <form action="{{ route('login.post') }}" method="POST" class="mt-7 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" placeholder="Въведи парола" class="min-h-12 w-full rounded-2xl border border-white/10 bg-white/10 px-4 py-4 text-white outline-none placeholder:text-white/40 focus:border-orange-300/50">
                        @error('password')
                            <p class="mt-2 text-sm text-red-300">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-between gap-4 text-sm text-white/60">
                        ...
                        <a href="{{ route('contact') }}" class="font-semibold text-orange-300 hover:text-white">Забравена парола?</a>
                    </div>
                ...
                <p class="mt-6 text-center text-sm text-white/60">
                    Нямаш профил?
                    <a href="{{ route('register') }}" class="font-bold text-orange-300 hover:text-white">Регистрирай се</a>
                </p>
            </div>
        </section>
    </main>
