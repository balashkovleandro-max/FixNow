        ...
        <div class="relative z-10 mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:px-8">
            <header class="flex flex-col gap-4 rounded-[2rem] border border-white/70 bg-white/75 p-4 shadow-2xl shadow-blue-900/5 backdrop-blur-2xl sm:flex-row sm:items-center sm:justify-between sm:p-5">
                <a href="{{ route('freelancer.jobs.index') }}" class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xl font-black">Детайл на обява</p>
                        <p class="text-sm text-slate-500">Баланс: {{ $creditStats['available'] }} кредита</p>
                    </div>
                </a>
                <a href="{{ route('freelancer.jobs.index') }}" class="rounded-2xl border border-slate-200 bg-white/80 px-5 py-3 text-sm font-black text-slate-700">Назад</a>
            </header>
            ...
            <section class="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
                <article class="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-2xl shadow-blue-900/10 backdrop-blur-2xl sm:p-8">
                    <p class="text-sm font-black uppercase tracking-[0.22em] text-violet-600">{{ $freelancerJob->category ?: 'Проект' }}</p>
                    <h1 class="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{{ $freelancerJob->title }}</h1>
                    <p class="mt-3 text-slate-500">{{ $freelancerJob->business?->business_name ?: $freelancerJob->business?->name }} @if($freelancerJob->location) · {{ $freelancerJob->location }} @endif</p>
                    ...
                    <div class="mt-6 grid gap-3 sm:grid-cols-3">
                        ...
                        <div class="rounded-3xl bg-fuchsia-50 p-4">
                            <p class="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-700">Цена</p>
                    ...
                    <div class="mt-8 prose prose-slate max-w-none">
                        <p class="whitespace-pre-line text-base leading-8 text-slate-700">{{ $freelancerJob->description }}</p>
                    </div>
                </article>

                <aside class="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-2xl shadow-fuchsia-900/10 backdrop-blur-2xl sm:p-8">
                    <p class="text-sm font-black uppercase tracking-[0.22em] text-blue-600">Кандидатстване</p>
                    <h2 class="mt-2 text-2xl font-black">Баланс: {{ $creditStats['available'] }} кредита</h2>
                    <p class="mt-3 text-sm leading-6 text-slate-600">При изпращане ще бъдат приспаднати {{ \App\Support\FreelancerCredits::APPLICATION_COST }} кредита.</p>
                        ...
                        <form action="{{ route('freelancer.jobs.apply', $freelancerJob) }}" method="POST" class="mt-6 grid gap-4">
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                <label class="grid gap-2 text-sm font-black text-slate-700">
                                    Предложена цена
                                    <input name="proposed_price" value="{{ old('proposed_price') }}" maxlength="120" placeholder="Напр. 450 € или по договаряне" class="min-h-12 rounded-2xl border border-slate-200 bg-white/80 px-4 text-slate-900 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100">
                                </label>
                                <label class="grid gap-2 text-sm font-black text-slate-700">
                                    Срок за изпълнение
                                    <input name="proposed_timeframe" value="{{ old('proposed_timeframe') }}" maxlength="120" placeholder="Напр. 7 дни" class="min-h-12 rounded-2xl border border-slate-200 bg-white/80 px-4 text-slate-900 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100">
                                </label>
                            </div>
                            <textarea name="cover_message" rows="6" placeholder="Кратко представяне, опит и как би помогнал по задачата..." class="rounded-3xl border border-slate-200 bg-white/80 px-4 py-4 text-sm leading-6 text-slate-900 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100">{{ old('cover_message') }}</textarea>
                            <button type="submit" class="min-h-12 rounded-2xl bg-gradient-to-r from-blue-600 via-violet-600 to-fuchsia-500 px-6 text-sm font-black text-white shadow-xl shadow-violet-500/25">Кандидатствай · 3 кредита</button>
                            <button type="button" data-open-credit-modal class="min-h-12 rounded-2xl border border-slate-200 bg-white/80 px-6 text-sm font-black text-slate-700">Купи кредити</button>
                        </form>
                    @endif
                </aside>
            </section>
